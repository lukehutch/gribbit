...
public class DataModelLoader {
    ...
    public static void checkFieldTypesAgainstAnnotations(Class<? extends DataModel> dataModelClass)
        ...
        for (Field field : dataModelClass.getFields()) {
            ...
            // Add value constraint checkers for class
                ...
                valueCheckers.add(new FieldValueConstraintChecker() {
                    ...
                    public void checkFieldsAgainstConstraints(DataModel dataModelObj) {
                        ...
                        if (fieldValue == null || (fieldValue instanceof String && ((String) fieldValue).isEmpty())) {
                            throw new RuntimeException("Required field " + dataModelClass.getName() + "." + fieldName
                                    + " is null or empty");
                        }
                    }
                });
            ...
            if (fieldType == String.class) {
                ...
                boolean normalizeSpacing = field.isAnnotationPresent(NormalizeSpacing.class);
                boolean isEmail = field.isAnnotationPresent(Email.class);
                boolean needToTrim = !field.isAnnotationPresent(NoTrim.class) || isEmail;
                boolean toLowerCase = isEmail;
                boolean checkMinLength = field.isAnnotationPresent(MinLength.class);
                int minLength = checkMinLength ? ((MinLength) field.getAnnotation(MinLength.class)).value() : 0;
                boolean checkMaxLength = field.isAnnotationPresent(MaxLength.class);
                ...
                if (normalizeSpacing || needToTrim || toLowerCase || checkMinLength || checkMaxLength) {
                    valueCheckers.add(new FieldValueConstraintChecker() {
                        @Override
                        public void checkFieldsAgainstConstraints(DataModel dataModelObj) {
                            Object fieldValue;
                ...
                boolean checkMinValue = field.isAnnotationPresent(MinIntegerValue.class);
                int minValue =
                        checkMinValue ? ((MinIntegerValue) field.getAnnotation(MinIntegerValue.class)).value() : 0;
                boolean checkMaxValue = field.isAnnotationPresent(MinIntegerValue.class);
                int maxValue =
                        checkMaxValue ? ((MaxIntegerValue) field.getAnnotation(MaxIntegerValue.class)).value() : 0;
