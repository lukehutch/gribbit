...
public interface VulcanizedJSHandler extends AuthNotRequiredRoute {
    public default Response get() throws Exception {
        ...
        if (!getRequest().cachedVersionIsOlderThan(resourcesLoadedEpochSeconds)) {
            ...
            ByteBuf vulcanizedJS = GribbitServer.siteResources.getVulcanizedJS();
            // Have to increase refcount each time the resource is served
            vulcanizedJS.retain();
            ByteBufResponse response =
                    new ByteBufResponse(HttpResponseStatus.OK, "application/javascript;charset=utf-8", vulcanizedJS);
